 ...
 * Implements queen-specific movement rules (combination of rook and bishop movements).
 */
public class Queen extends ChessPiece {
    // All possible directions (combination of orthogonal and diagonal)
    private static final int[][] ALL_DIRECTIONS = {
        {0, 1}, {0, -1}, {1, 0}, {-1, 0},  // Orthogonal (rook-like)
        {1, 1}, {1, -1}, {-1, 1}, {-1, -1} // Diagonal (bishop-like)
    };

    public Queen(Color color, String position, GameState gameState) {
    ...
    public boolean isValidMove(Move move) {
        ...
        // Queen must move either like a rook (one difference is 0) or like a bishop (absolute differences are equal)
    ...
    public String[] getLegalMoves() {
        ...
        for (String targetSquare : getAttackedSquares()) {
            Move move = new Move(position, targetSquare, PieceType.QUEEN);
            if (isValidMove(move)) {
